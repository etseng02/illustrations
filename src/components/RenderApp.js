...
function RenderApp() {
  ...
  const generateRandomString = function () {
    ...
    while (newShortURL.length < 4) {
      newShortURL =
        newShortURL + possibleCharacters[Math.floor(Math.random() * 34)]
    }
    return newShortURL
  }
  ...
  function startGame() {
    //TRIGGERS AFTER ROOM HAS BEEN CREATED. ONLY AVAILABLE ON HOST MACHINE
    setState(prevState => ({ ...prevState, round: 0, ready: [] }))
    socket.emit("startGame", state.roomID)
  }

  function ready() {
    //AVAILABLE ON MOBILE DEVICES ON CLIENT WAITING PAGE, DRAWING PAGE, AND GUESS PAGE
  ...
  useEffect(() => {
    //Add player name to lobby by adding player to players array in state
  ...
  useEffect(() => {
    //Add player name to ready state array when received
  ...
  useEffect(() => {
    //Automatically start the next round once all players are ready.
    console.log(state.players, state.ready)
    if (
      state.ready.length === state.players.length &&
      state.gameID &&
      state.hostMachine === true
    ) {
      socket.emit("nextRound", state.gameID, state.round, state.roomID)
    }
  }, [state.ready])
  ...
  useEffect(() => {
    //Start the game when command has been received. Display prompt, set round to 0.
  ...
  useEffect(() => {
    //Receive player position after attempting to join room. Will reset state if duplicate name is received
    socket.on("joinRoom", function (name, position) {
      console.log(
        `Receiving a player position for ${name} and assigning ${position}`,
      )
      if (
        name === state.name &&
        position === "error" &&
  ...
  useEffect(() => {
    //once the name in state has successfully changed during Join Room, send to the server the name and roomID
    if (state.name) {
      console.log(
        "the name state has changed, sending to the server a joinRoom Command",
      )
  ...
  useEffect(() => {
    socket.on("nextRoundInfo", function (submissionData) {
      //content, promptID, roundID, playerposition  received in array of arrays.
      console.log(
        "I have received the content to start the next round",
        submissionData,
      )
      if (submissionData) {
  ...
  return (
    <Fragment>
      <Header room={state.roomID}></Header>
      ...
      {state.phase === "draw" &&
      !state.hostMachine && ( //Client Draw Phase
          <Fragment>
            <h3 id="draw-this-title" style={{ textAlign: "center" }}>
              Draw this: {state.prompt}
            </h3>

            <Canvas
              ref={ref => (canvasData.current = ref)}
      ...
      {state.phase === "guess" &&
      !state.hostMachine && ( //Client Guess Phase
          <Fragment>
            <Guess
              ready={ready}
              setGuess={setState}
      ...
      {state.roomID &&
      state.hostMachine && ( //HostMachine requires RoomID to properly excecute the game
          <Fragment>
            <HostRoom
              ready={state.ready}
              roomID={state.roomID}
      ...
      {!state.roomID &&
      !state.hostMachine && ( //If client does not have a roomID, Client is in homescreen
          <Fragment>
            <HomeScreen
              onClick={enterRoom}
      ...
      {state.roomID &&
      !state.hostMachine &&
      state.phase === "" && ( //If the client receives room ID and is not the host machine, put client in waiting room
          <Fragment>
            <Waiting
              name={state.name}
              room={state.roomID}
      ...
      {!state.hostMachine &&
      state.phase === "loading" && ( //Client transtion from one round to another
          <Fragment>
            <Loading></Loading>
          </Fragment>
        )}
